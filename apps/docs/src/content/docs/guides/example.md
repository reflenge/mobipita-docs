...
```mermaid
graph TD
    User[ユーザー] --> Vercel[Vercel (Next.js Compute)]
    Vercel -- Auth --> Clerk[Clerk (認証/ユーザー管理)]
    Vercel -- DB Query --> Neon[Neon/Supabase (PostgreSQL)]
    Vercel -- Asset/Image --> R2[Cloudflare R2 (オブジェクトストレージ)]

    subgraph "External Services (Managed)"
        Clerk
        Neon
        R2
    end
...
**AWSへ移行した後もコードを書き直さずに済む**よう、以下のルールを厳守してください。
